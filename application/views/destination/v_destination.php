  ...
  <div class="content-wrapper">
                  ...
                  <div class="col-md-12">
                      <div class="card card-primary">
                          <div class="card-header">
                              Manage Data Destination
                          </div>
                          <div class="card-body">
                              <div class="row">
                                  <div class="col-md-3">
                                      <button class="btn btn-primary" id="btn_add_destination">
                                          <i class="fa fa-plus"></i> Add New Destination
                                      </button>
                                  </div>
                                  <div class="col-md-12 mt-3">
                                      <table class="table" id="table_destination">
                                          <thead>
                                              <tr>
                                                  <th>Name</th>
                                                  <th>Action</th>
      ...
      <!-- Modal -->
      <form id="form_destination" method="POST">
          <div class="modal fade" id="modal_destination" tabindex="-1" role="dialog">
  ...
  <script>
      $(document).ready(function() {
          ...
          var table = $('#table_destination').DataTable({
              ajax: {
                  url: "<?= site_url('destination/get_ajax_data_destination') ?>",
                  type: 'POST'
              },
              order: [],
              "responsive": true,
          ...
          $('#btn_add_destination').on('click', function() {
              $('#modal_destination .modal-title').html('Add New Destination');
              if (mode == 'edit') {
                  $('#id').val('').change();
                  $('#name').val('').change();
              }
          ...
          $('#table_destination').on('click', '.btn-edit', function() {
              mode = 'edit';
              ...
              $('#modal_destination .modal-title').html('Edit Data Destination');
              $('#modal_destination').modal();
          })

          $('#table_destination').on('click', '.btn-delete', function() {
              var row = table.row($(this).parent().parent()).data();
              if (confirm('Are you sure want to delete this data ?')) {
                  $.ajax({
                      url: "<?= site_url('destination/delete_data') ?>",
                      data: {
                          id: row['id']
                      },
                      type: 'POST',
          ...
          $('#form_destination').on('submit', function(e) {
              ...
              if (confirm('Are you sure want to submit this data ?')) {
                  ...
                  $.ajax({
                      url: "<?= site_url('destination/save_data') ?>",
                      data: formData,
                      processData: false,
                      contentType: false,
                      async: false,
